...
try {
    ...
    if (-not (Test-Path $itachiKeyFile)) { exit 0 }
...
(async () => {
    ...
    try {
        ...
        try {
            ...
            // Can't proceed without remote list
...
"@
    ...
    $output = node -e $nodeScript $itachiKeyFile $SYNC_API $claudeDir 2>&1
    ...
    try {
        Add-Content -Path $logFile -Value "[$ts] SKILL SYNC ERROR: $($_.Exception.Message)"
    } catch {}
}
