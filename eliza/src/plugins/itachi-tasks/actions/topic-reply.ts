...
import { activeSessions, isSessionTopic, spawningTopics } from '../shared/active-sessions.js';
...
export const topicReplyAction: Action = {
  ...
  validate: async (runtime: IAgentRuntime, message: Memory): Promise<boolean> => {
    ...
    // Session/browsing/spawning topics are handled exclusively by topic-input-relay
    if (activeSessions.has(threadId) || browsingSessionMap.has(threadId)
        || isSessionTopic(threadId) || spawningTopics.has(threadId)) return false;
