...
(async () => {
    try {
        ...
        for (const f of list.files) {
            ...
            fs.mkdirSync(path.dirname(localPath), { recursive: true });
            fs.writeFileSync(localPath, remoteContent);
            output.push('[sync] Updated ' + f.file_path.replace(/\//g, '\\') + ' (v' + f.version + ' by ' + f.updated_by + ')');
        }
        if (output.length > 0) console.log(output.join('\n'));
    } catch(e) {}
})();
"@
        $globalSyncOutput = node -e $globalNodeScript $itachiKeyFile $SYNC_API $globalSyncTarget 2>$null
...
const briefingJson = process.argv[3];
const learningsJson = process.argv[4];
...
try {
    ...
    // Determine target file based on client
    let targetFile;
    if (client === 'claude') {
        // Claude: write to ~/.claude/projects/{encoded-cwd}/memory/MEMORY.md
    ...
    if (lines.length > 3) {
        lines.push('');
        existing = upsertSection(existing, '## Itachi Session Context', lines.join('\n'));
    }
...
"@
        ...
        if ($briefingJsonArg -or $learningsJsonArg) {
            node -e $contextScript $client $cwd $briefingJsonArg $learningsJsonArg 2>$null
        }
    } catch {}
}
catch {
